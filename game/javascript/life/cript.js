...
let column = cns.offsetHeight;
let strik = cns.offsetWidth;
...
let col = Math.floor(cns.offsetHeight / wisize);
let str = Math.floor(cns.offsetWidth / wisize);
...
function drawLife() {
    for (let i = 0; i < col; i++) {
        com[i] = [];
        for (let j = 0; j < str; j++) {
            if ((Math.round(Math.random() * 100) > 77)) {
                com[i][j] = 1;
            } else {
                com[i][j] = 0;
            }
